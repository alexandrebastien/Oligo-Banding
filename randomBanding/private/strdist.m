function d = strdist(r, b, krk, cas)
    ...
    %
    % Copyright (c) 2006, Eduard Polityko. All rights reserved.
    % --> See full copyright notice here: https://bit.ly/38YP3tK
    ...
    switch nargin
       ...
       case 4
          bb = b; rr = r;
          if cas > 0; bb = upper(b); rr = upper(r); end
    end

    if krk ~= 2; krk = 1; end

    d = []; luma = numel(bb); lima = numel(rr);
    ...
    for krk1 = 1:krk
        for i = 2:lu1
           ...
           for j = 2:li1
              kr = krk1;
              if strcmp(rr(j-1),bbi); kr = 0; end
              dl(i,j) = min([dl(i-1,j-1)+kr,dl(i-1,j)+1,dl(i,j-1)+1]);
           end
        end
        d = [d dl(end,end)]; %#ok<AGROW>
    end
end